// server/src/controllers/archivos.pacientes.controller.js
import fs from "fs";
...
const UPLOAD_ROOT = path.resolve(process.cwd(), "server", "uploads", "pacientes");
...
export async function subirArchivo(req, res, next) {
  try {
    ...
    const nombreVisible = file.originalname;
    const { rows } = await query(
      `INSERT INTO paciente_archivos
        (id, paciente_id, nombre, filename_orig, storage_key, mime, bytes)
...
export async function descargarArchivo(req, res, next) {
  try {
    ...
    const { rows } = await query(
      `SELECT filename_orig, storage_key, mime
       FROM paciente_archivos
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${encodeURIComponent(filename_orig)}"`
    );
...
// Opcional: renombrar archivo (si quieres soporte PATCH)
